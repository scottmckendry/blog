...
Now I'm fairly certain this is a _"solved"_ problem. There are probably a bunch of tools and scripts that can do this for me. But sometimes _[reinventing the wheel](https://xkcd.com/1673/)_ is more fun.
...
However, the child process _could_ modify a local file, which the parent process could then read. So instead of setting the environment variable directly, we can write code to export the variable to `.bash_profile`. I can then source this file as part of the prompt command.
...
But now we have a new problem. For some reason, the `apt` command exhibits strange behaviour when run in the background. To the point where the terminal would become **_unusable_**. Redirecting output to `/dev/null` didn't help.
...
_Et voila!_ We have a working solution for both dotfile changes and pending software updates in Bash.
...
That's where PowerShell's _secret weapon_ comes in: `Start-ThreadJob`. It creates background jobs similar to `Start-Job`, but with one key difference: the jobs created run in separate threads within the local process.
...
_What is it with package managers and being obnoxious?_ 🙃
